...
for i in range(len(your_btc_address)): #each loop takes the next address to query from the list of addresses
    ...
    df = pd.read_json(transactions_url) # reads page
    transactions = df['txs'] # gets transactions
    while (transactions.count() > df['n_tx'][0] - 1 or transactions.count() > 4999) and exit_while == False: # until the items on the page are less than 4999 (implies last page is reached) or num of transactions on the page are less than total transactions overall - and until exit while is true
        ...
        if df['n_tx'][0] > 5000:    
            offset = offset + 5000   # incriments offfset so page is reloaded with the next 100 transactions 
            transactions_url = url % (your_btc_address[i],"?format=json&limit=5000&offset=", offset) #-new-
            time.sleep(20)
            df = pd.read_json(transactions_url, 'utf-8-sig') # reads page
            transactions = df['txs'] # gets transactions
        time.sleep(20) #query rate  limit is 10 seconds
    exit_while = False
